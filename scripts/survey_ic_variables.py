#!/usr/bin/env python3
"""Survey the initial-condition netCDF files and report their variable sets.

The initial-condition ensemble is one small netCDF file per site and member,
laid out as ``<root>/<site>/IC_site_<site>_<member>.nc``. The files are not all
alike: some are reported to carry ``leaf_carbon_content`` and ``SoilMoistFrac``
in addition to the three variables seen so far. Ingest has to know the full set
of combinations before it can build a ``(member, site)`` array, and that
question can only be answered where the files are, which is the SCC.

This script answers it. It reports, across every file it finds:
...
* whether the site number in the directory matches the one in the file name;
* whether ``AbvGrndWood`` and ``wood_carbon_content`` are always equal, and how
  often the ``-999.0`` fill value appears.

It has **no third-party dependencies**. The files are netCDF-3 classic
(``CDF\\x01``), whose header format is simple enough to parse directly, so this
runs under any Python 3 with no modules loaded and no environment to activate.

Only the header and the handful of data values are read, about a kilobyte per
file, so the run is dominated by filesystem latency rather than by parsing.
That is why ``--jobs`` exists: on a networked filesystem, reading files
concurrently is worth roughly an order of magnitude.

Examples
--------
Quick look at 200 randomly chosen sites::

    python3 survey_ic_variables.py --root /path/to/IC/IC --sample 200

The full survey, written to a JSON file to send on::

    python3 survey_ic_variables.py --root /path/to/IC/IC --jobs 16 \\
        --out ic_survey.json
"""
    ...
    with ThreadPoolExecutor(max_workers=jobs) as pool:
        for done, (path, outcome) in enumerate(pool.map(attempt, paths), start=1):
            if isinstance(outcome, Exception):
                totals.failures.append((str(path), f"{type(outcome).__name__}: {outcome}"))
            else:
                accumulate(totals, outcome, path)
            if done % 20000 == 0:
                print(f"  ... {done} of {len(paths)} files", file=sys.stderr, flush=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.sample is not None:
        ...
        chosen = set(random.Random(args.seed).sample(sites, min(args.sample, len(sites))))
        paths = [p for p in paths if p.parent.name in chosen]
        print(f"Sampling {len(chosen)} of {len(sites)} sites.", file=sys.stderr)

    print(f"Reading {len(paths)} files with {args.jobs} workers ...", file=sys.stderr, flush=True)
