...
class TestLoadDrivers:
    ...
    def test_the_time_axis_is_pysipnets(self, root, site_table):
    ...
    def test_the_clock_is_undeclared_unless_the_caller_declares_it(self, root, site_table):
        undeclared = load_drivers([3], root=root, site_table=site_table)
        assert undeclared["time"].attrs["time_zone"] == "undeclared"
        assert undeclared.attrs["time_zone"] == "undeclared"
        declared = load_drivers([3], root=root, site_table=site_table, time_zone="UTC")
    ...
    def test_the_dataset_attributes_are_pysipnets_plus_the_ensembles(self, root, site_table):
        dataset = load_drivers([3], root=root, site_table=site_table)
        own = read_driver_file(driver_file(root, 3, 1)).xarray
    ...
    def test_sites_come_back_in_the_order_given(self, root, site_table):
        dataset = load_drivers([7, 3], root=root, site_table=site_table)
        np.testing.assert_array_equal(dataset["site"].values, [7, 3])
        np.testing.assert_array_equal(
            dataset["lon"].values, site_table.set_index("site_id").loc[[7, 3], "lon"].values
        )
        ascending = load_drivers([3, 7], root=root, site_table=site_table)
    ...
    def test_rejects_a_non_positive_source_index(self, root, site_table):
        with pytest.raises(ValueError, match=r"source_indices\[0\] must be at least 1"):
            load_drivers([3], source_indices=[0], root=root, site_table=site_table)

    @pytest.mark.parametrize("indices", [[1.5], [2.0], "12", 2, ["1", "2"], [True], [np.inf]])
    def test_rejects_source_indices_that_are_not_integers(self, root, site_table, indices):
        with pytest.raises(TypeError, match="source_indices"):
            load_drivers([3], source_indices=indices, root=root, site_table=site_table)

    def test_a_source_index_beyond_int64_is_a_value_error(self, root, site_table):
        """``2**70`` raised numpy's raw OverflowError."""
        with pytest.raises(ValueError, match=r"source_indices\[0\] must be"):
            load_drivers([3], source_indices=[2**70], root=root, site_table=site_table)

    def test_the_member_coordinates_carry_their_attributes(self, root, site_table):
        from sipnet_calibration.conventions import (
    ...
    def test_rejects_no_source_indices(self, root, site_table):
    ...
    def test_rejects_sites_that_are_not_integers(self, root, site_table, sites):
        with pytest.raises(TypeError, match="sites"):
            load_drivers(sites, root=root, site_table=site_table)

    @pytest.mark.parametrize("sites", [[2**31], [0], [], [3, 3]])
    def test_rejects_sites_that_are_not_positive_whole_numbers_named_once(
        self, root, site_table, sites
    ):
        with pytest.raises(ValueError, match="site"):
            load_drivers(sites, root=root, site_table=site_table)

    def test_rejects_an_unusable_site_table(self, root, site_table):
        with pytest.raises(ValueError, match="no 'site_id' column or index"):
            load_drivers([3], root=root, site_table=site_table.drop(columns=["site_id"]))
        for column in ("lon", "lat"):
            with pytest.raises(ValueError, match=f"no \\['{column}'\\] column"):
                load_drivers([3], root=root, site_table=site_table.drop(columns=[column]))
        with pytest.raises(TypeError, match="must be a DataFrame"):
            load_drivers([3], root=root, site_table=site_table.to_dict())
        duplicated = pd.concat([site_table, site_table.head(3)], ignore_index=True)
        with pytest.raises(ValueError, match="more than once"):
            load_drivers([3], root=root, site_table=duplicated)

    def test_a_site_table_keyed_on_site_id_is_accepted(self, root, site_table):
        keyed = load_drivers([3], root=root, site_table=site_table.set_index("site_id"))
        plain = load_drivers([3], root=root, site_table=site_table)
        assert keyed.identical(plain)

    def test_lon_and_lat_carry_their_cf_attributes(self, root, site_table):
        dataset = load_drivers([3, 7], root=root, site_table=site_table)
        assert dataset["lon"].attrs == dict(LON_ATTRIBUTES)
        assert dataset["lat"].attrs == dict(LAT_ATTRIBUTES)
        expected = site_table.set_index("site_id").loc[[3, 7]]
    ...
    def test_site_carries_the_shared_site_attributes(self, root, site_table):
        dataset = load_drivers([3, 7], root=root, site_table=site_table)
    ...
    def test_non_physical_values_are_counted_not_altered(self, tmp_path, site_table):
        ...
        with warnings.catch_warnings():
            ...
            dataset = load_drivers([3], root=tmp_path, site_table=site_table)
    ...
    def test_an_invalid_clock_is_refused_before_any_file_is_read(
        self, root, site_table, time_zone
    ):
        with pytest.raises(ValueError, match="time_zone must be") as refusal:
            load_drivers([3], root=root, site_table=site_table, time_zone=time_zone)
        assert ".clim" not in str(refusal.value)

    def test_aggregation_keeps_the_declared_clock(self, root, site_table):
        dataset = load_drivers([3], root=root, site_table=site_table, time_zone="UTC")
    ...
    def test_no_coordinate_is_written_with_a_fill_value(self, root, site_table, tmp_path):
        """CF forbids ``_FillValue`` on a coordinate."""
        path = tmp_path / "drivers.nc"
        load_drivers([3], root=root, site_table=site_table).to_netcdf(path)
    ...
    def test_driver_present_is_mapped(self, root, site_table):
    ...
    def test_driver_present_is_all_true_when_nothing_is_missing(self, root, site_table):
        ...
        assert dataset.attrs["coverage"] == "complete"
        assert DRIVER_PRESENT not in load_drivers([3, 7], root=root, site_table=site_table)

    def test_raises_when_no_file_exists_at_all(self, root, site_table):
        with pytest.raises(FileNotFoundError, match="no driver directories"):
            load_drivers([4], root=root, site_table=site_table)
        with pytest.raises(FileNotFoundError, match="no driver files"):
            load_drivers([3], source_indices=[9], root=root, site_table=site_table, allow_missing=True)

    def test_rejects_a_missing_root(self, tmp_path, site_table):
    ...
    def test_rejects_file_name_dates_that_do_not_match_the_data(self, root, site_table, name):
        path = driver_file(root, 3, 2)
    ...
    def test_rejects_a_file_name_off_the_template(self, root, site_table):
        path = driver_file(root, 3, 2)
...
class TestDriverFields:
    ...
    def test_nothing_points_at_a_bounds_variable_a_field_cannot_carry(self, root, site_table):
        dataset = load_drivers([3], root=root, site_table=site_table)
    ...
    def test_fields_carry_the_variable_attributes(self, root, site_table):
        dataset = load_drivers([3], root=root, site_table=site_table)
        fields = driver_fields(dataset)
        for name, field in fields.items():
    ...
    def test_rejects_a_dataset_missing_a_variable(self, root, site_table):
        dataset = load_drivers([3], root=root, site_table=site_table).drop_vars("wind_speed")
...
class TestRealFiles:
    ...
    def test_the_local_files_do_not_load(self):
        ...
        with pytest.raises(ValueError, match="pySIPNET refused the file"):
            load_drivers(
                sites, source_indices=members, root=REAL_ROOT, site_table=load_sites(),
                allow_missing=True,
            )
    ...
    def test_a_run_and_its_drivers_share_one_time_axis(self, real_drivers, site_1_result):
        ...
        nee = to_model_output(site_1_result, output_variable_names=["nee"])["net_ecosystem_exchange"]
